...
def update_cron(create_item, state, settings):
    ...
    div = BeautifulSoup(html, "html.parser").find('div', {'class': 'project-nav__links'})
    data = div.findAll('a')
    new_state = {el.attrs['data-content']: int(el.find('span').text.replace(",","")) for el in data if len(el.findAll('span')) > 0}
